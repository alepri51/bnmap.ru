    ...
    <widget name="auth">
        <v-card>
            <v-layout column fill-height>
                <v-card-title>
                    <h2 class="widget-caption"><v-icon class="mr-1 primary--text">fas fa-wallet</v-icon>Ваш счет:</h2>
                </v-card-title>

                <v-divider/>

                <v-card-text style="flex: 1">
                    <div class="subheading"><v-icon small class="mr-1 green--text">fas fa-dollar-sign</v-icon>Наименование: {{filter.name}}</div>
                    <div class="subheading"><v-icon small class="mr-1 green--text">fas fa-dollar-sign</v-icon>Адрес: {{filter.address}}</div>
                </v-card-text>
...
<script>
    ...
    export default {
        ...
        computed: {
            filter() {
                //debugger;
                return this.raw_data.filter(item => item.member === this.auth.member && item.default)[0] || {};
            }
        },
    }
</script>
